...
@Component({
    ...
    <section class="page-banner">
      <div class="container">
        ...
        <nav class="crumbs">
          ...
          <a routerLink="/news">الأخبار</a> <span>›</span>
          إدارة
        </nav>
      </div>
    </section>
    ...
    <section class="section">
      <div class="container">
        <div class="toolbar">
          <div>
            <h2>قائمة الأخبار</h2>
            <p class="hint">يمكنك إضافة، تعديل، أو حذف الأخبار الظاهرة في الصفحة.</p>
          </div>
          <div class="toolbar-actions">
            <button class="btn btn-primary" (click)="openNew()">+ إضافة خبر</button>
        ...
        @if (loading()) {
          ...
          <p class="state">لا توجد أخبار بعد. ابدأ بإضافة خبر.</p>
        } @else {
            ...
            @for (n of items(); track n.id) {
              <article class="row-card">
                ...
                <div class="info">
                  ...
                  <h3>{{ n.title }}</h3>
                  <p class="meta">{{ formatDate(n.date) }}</p>
                  <p class="excerpt">{{ n.excerpt }}</p>
                  <div class="row-actions">
                    <button class="btn btn-outline btn-sm" (click)="openEdit(n)">تعديل</button>
                    <button class="btn btn-danger btn-sm" (click)="onDelete(n)">حذف</button>
                  </div>
                </div>
              </article>
            }
      ...
      <div class="modal-backdrop" (click)="closeForm()">
        <div class="modal" (click)="$event.stopPropagation()">
          <header class="modal-head">
            <h3>{{ editingId() ? 'تعديل خبر' : 'إضافة خبر' }}</h3>
          ...
          <form class="modal-body" (ngSubmit)="onSave()" #f="ngForm">
            ...
            <div class="grid-2">
              <label>
                <span>التصنيف</span>
                <input name="category" [(ngModel)]="form.category" placeholder="حملات، مشاريع، أخبار..." required />
              </label>
              <label>
                <span>التاريخ</span>
                <input name="date" type="date" [(ngModel)]="form.date" required />
              </label>
            </div>

            <label>
              <span>المقتطف</span>
              <textarea name="excerpt" [(ngModel)]="form.excerpt" rows="2" required></textarea>
            </label>

            <label>
              <span>النص الكامل <small>(اختياري)</small></span>
              <textarea name="body" [(ngModel)]="form.body" rows="4"></textarea>
            </label>

            <div class="image-field">
              <span class="lbl">صورة الخبر</span>
  ...
  styles: [`
    ...
    .excerpt { margin: 0; color: var(--color-text); font-size: 13px; line-height: 1.7; }
...
export class AdminNewsComponent implements OnInit {
  ...
  async onSave() {
    if (!this.form.image && !this.pendingFile) {
      this.formError.set('الرجاء اختيار صورة للخبر.');
      return;
    }

    if (!this.editingId() && !this.form.slug) {
      this.form.slug = `news-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    }
    ...
    try {
      if (this.pendingFile) {
        ...
        this.form.image = await this.newsService.uploadImage(this.pendingFile);
        this.uploading.set(false);
      }

      const id = this.editingId();
      if (id) {
        await this.newsService.update(id, this.form);
      } else {
        await this.newsService.create(this.form);
      }
      this.showForm.set(false);
      await this.reload();
    } catch (e: any) {
  ...
  async onDelete(n: NewsItem) {
    if (!confirm(`هل أنت متأكد من حذف "${n.title}"؟`)) return;
    try {
      await this.newsService.remove(n.id!);
      await this.reload();
    } catch (err: any) {
